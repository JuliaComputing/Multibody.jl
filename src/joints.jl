...
@component function UniversalSpherical(; name,
    ...
    systems = @named begin
        ...
        frame_b = Frame()
        frame_ia = Frame()
        # R_rel_ia1 = NumRotationMatrix()
        # R_rel_ia2 = NumRotationMatrix()
    ...
    Ria = absolute_rotation(ori(frame_a), R_rel_ia)

    eqs = [

        if kinematic_constraint
            rRod_0 .~ ori(frame_b).R'*(ori(frame_b).R*collect(frame_b.r_0)) - ori(frame_a).R'*(ori(frame_a).R*collect(frame_a.r_0))
        else
            rRod_0 .~ frame_b.r_0 - frame_a.r_0
        end
        ...
        der_rRod_a_L .~ (resolve2(ori(frame_a), D.(rRod_0)) - cross(ori(frame_a).w, rRod_a))/rodLength
        ...
        frame_b.f .~ resolve_relative(f_b_a, ori(frame_a), ori(frame_b))
        frame_b.tau .~ 0
