...
@component function FurutaPendulum(; name)
    systems = @named begin
        ...
        elbow_joint    = Revolute(n = [0, 0, 1], isroot = true, axisflange = true, phi=0.1)
...
prob = ODEProblem(ssys, [
    ssys.shoulder_joint.phi => 0.0,
    ssys.elbow_joint.phi => 0.1,
    ssys.shoulder_joint.w => 0.0,
    ssys.elbow_joint.w => 0.0,
    Multibody.WORLD_G => 9.81], (0, 12))
...
@test rb1 ≈ [-0.2836231361058395, 0.0, 0.528732367711197] atol=1e-3

# If we look at the variable `ssys.upper_arm.r`, we do not see this rotation!
arm_r = sol(1, idxs=collect(ssys.upper_arm.r))
@test arm_r == [0, 0, 0.6]
...
@test get_rot(sol, ssys.upper_arm.frame_a, 1)*arm_r ≈ rb1
...
t1 = get_trans(sol, ssys.lower_arm.frame_b, 12)
@test t1 ≈ [-0.009040487302666853, -0.59999996727278, 0.599931920189277] rtol=1e-2
...
t1rot = get_rot(sol, ssys.lower_arm.frame_a, 12)*sol(12, idxs=collect(ssys.lower_arm.r))
...
r_A = sol(12, idxs=collect(ssys.lower_arm.r))
r_A = [r_A; 1] # Homogeneous coordinates

@test (get_frame(sol, ssys.lower_arm.frame_a, 12)*r_A)[1:3] ≈ t1

# the vector is now coinciding with `get_trans(sol, ssys.lower_arm.frame_b, 12)`.

@test get_trans(sol, ssys.tip.frame_a, 12) ≈ t1
@test get_frame(sol, ssys.tip.frame_a, 12)[1:3, 4] ≈ t1